  ...
  <div class="main-box">
    <div :class="['container', 'container-register', { 'is-txl': isLogin }]">
      <el-form>
        ...
        <input v-model="registerForm.username" @input="validateUsername" class="form__input" type="text"
          placeholder="Name" />
        <span v-if="errors.username" class="form-item__error">{{ errors.username }}</span>

        <input v-model="registerForm.password" @input="validatePassword" class="form__input" type="password"
          placeholder="Password" />
        <span v-if="errors.password" class="form-item__error">{{ errors.password }}</span>

        <div class="primary-btn" @click="toRegister">立即注册</div>
      </el-form>
    </div>
    <div :class="['container', 'container-login', { 'is-txl is-z200': isLogin }]">
      <form>
        ...
        <input class="form__input" type="password" placeholder="Password" @keyup.enter="toLogin"
          v-model="loginForm.password" />
        <div class="primary-btn" @click="toLogin">立即登录</div>
      </form>
    </div>
...
const validateUsername = () => {
  if (registerForm.value.username.length < 3 || registerForm.value.username.length > 15) {
    errors.username = '用户名需3-15个字符';
  } else if (!/^[A-Za-z0-9]+$/.test(registerForm.value.username)) {
